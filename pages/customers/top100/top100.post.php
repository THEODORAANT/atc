      ...
      if (Util::count($customers)) {
          ...
          foreach($customers as $Customer) {
            ...
            echo '<tr>';
              ...
              echo '<td><span class="glyphicon '.($Customer->regdev() ? 'glyphicon-ok' : '').'"></span></td>';
              echo '<td>'.$Customer->customerFirstOrder().'</td>';

              $t = strtotime($Customer->last_order());

              if ($t < strtotime('-3 MONTHS') && $Customer->stockpile()=='0') {
              	echo '<td style="color: red;">';
              }else{
              	echo '<td>';
              }

              echo date('Y-m-d', $t).'</td>';
